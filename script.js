// script.js - Gestion du stock de câbles avec FIRESTORE
...
function initApp() {
  ...
  async function loadData() {
    try {
      // Charger Face P
      const docRefP = doc(db, 'stock', 'P');
      const docSnapP = await getDoc(docRefP);
      
      // Charger Face Q
      const docRefQ = doc(db, 'stock', 'Q');
      const docSnapQ = await getDoc(docRefQ);
      
      if (!docSnapP.exists() || !docSnapQ.exists()) {
        console.log("Base vide → création des données initiales");
        // Créer les documents initiaux
        await setDoc(doc(db, 'stock', 'P'), initialData.P);
        await setDoc(doc(db, 'stock', 'Q'), initialData.Q);
        stockData = initialData;
      } else {
        stockData.P = docSnapP.data();
        stockData.Q = docSnapQ.data();
      }
      ...
      alert("Erreur de connexion à Firebase. Vérifiez les règles Firestore !");
    }
  }

  // Écouter les changements en temps réel
  const unsubscribeP = onSnapshot(doc(db, 'stock', 'P'), (docSnap) => {
    if (docSnap.exists()) {
      stockData.P = docSnap.data();
      renderStock();
      updateStats();
    }
  });

  const unsubscribeQ = onSnapshot(doc(db, 'stock', 'Q'), (docSnap) => {
    if (docSnap.exists()) {
      stockData.Q = docSnap.data();
      renderStock();
      updateStats();
    }
  });
  ...
  async function saveData(face) {
    try {
      await setDoc(doc(db, 'stock', face), stockData[face]);
      console.log(`Face ${face} sauvegardée`);
    } catch (error) {
      console.error("Erreur de sauvegarde:", error);
      alert("Erreur de sauvegarde. Vérifiez les règles Firestore !");
    }
  }
